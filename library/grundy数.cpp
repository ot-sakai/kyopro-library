//Nim
//grundy数のxorsumが0の場合→後手勝ち
//xorsumが0でない場合→先手勝ち
//grundy数の求め方はメモ化再帰dpをする
...
int main() {
    ...
    auto dfs = [&](auto dfs, int x) -> int {
        ...
        if(x == 1) return 0; //grundy数0が確定の場合
        ...
        return dp[x] = st.mex(); //遷移先のmexがgrundy数となる
    };
    ...
    if(xorsum != 0) cout << "sente" << endl; //先手勝ち
    else cout << "gote" << endl; //後手勝ち
}